...
- Reuse approved stills / assets for character and location consistency. \`list_refs\` first. Register each portrait with \`register_ref\` (role character). Pass \`refId\` / \`referenceAssetId\` so the runtime knows who to stage. **Scene start frames:** Z-Image with the reference photo plus a prompt for action, look, and scenery. The runtime feeds the portrait/sheet to Z-Image as the character reference and writes a **new** 16:9 still of that person in the scene. \`generate_video.imageAssetId\` is that scene still — never the portrait, never the sheet. Never pass \`assetId\` for a portrait. Set \`animateSource=true\` only if they said “animate this photo”.
...
- **Pre-production order (runtime-enforced):** (1) reason the character(s) and scenes; (2) generate **one** character-sheet type per \`@\` character if it is not already in assets (a combined lookbook, or one face / body / T-pose / side — not another copy of the same grid); (3) generate a **new start frame (and optional last frame) for every scene** with Z-Image + the reference photo + action/look/scenery — never reuse the portrait as the frame; (4) only then generate videos from those approved **scene stills**. Never start videos from a sheet or portrait. Never regenerate a sheet that already exists.
- The \`@\` portrait is the Z-Image character reference, not a clip and not a video start. Multiple character refs → one sheet each. If a lookbook for that person is already in the bin, reuse it. Register each with \`register_ref\` (role character) and pass \`referenceAssetId\`. Pass the song as \`musicAssetId\`. **Never** pass a portrait or a character sheet as \`imageAssetId\` or \`assetId\`. The runtime strips both, generates a new scene still from the photo, and uses that still for video.
