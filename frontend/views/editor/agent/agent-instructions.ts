...
- Reuse approved stills / assets for character and location consistency. \`list_refs\` first. Register a portrait with \`register_ref\` (role character). Pass \`refId\` / \`referenceAssetId\` so the runtime knows who to stage. Z-Image has no face-lock API — a portrait passed as \`imagePath\` is an **edit of that photo**, so identity stills are generated as **new text-to-image scenes** (same person described in the prompt, new pose and setting). Then \`generate_video.imageAssetId\` is that new still — never the original portrait unless they asked to animate that exact photo.
...
- The \`@\` portrait is a character reference, not a clip. Register it with \`register_ref\` (role character) and pass \`referenceAssetId\`. Pass the song as \`musicAssetId\`. **Never** pass that portrait as \`imageAssetId\`. The runtime strips it, then generates a new lookbook and new start frames as text-to-image (Z-Image cannot face-lock; img2img would reprint the headshot).
