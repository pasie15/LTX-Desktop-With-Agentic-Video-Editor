...
export const GENERATE_TOOL_DEFINITIONS: AgentToolDeclaration[] = [
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        startTime: { type: 'number' },
        confirmed: { type: 'boolean' },
        referenceAssetId: { type: 'string', description: 'Character reference photo or generated scene still. A portrait/sheet is fed to Z-Image as the character reference for a NEW scene still (action, look, scenery). It is never the video start frame.' },
        refId: { type: 'string', description: 'Named ref from list_refs; resolved to an image asset. Identity only, never img2img.' },
        identityReference: { type: 'boolean', description: 'This still is a character reference. Z-Image uses the photo to restage the person in a new scene. Implied for imported portraits and sheets.' },
