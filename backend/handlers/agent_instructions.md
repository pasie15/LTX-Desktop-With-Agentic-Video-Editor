...
- **Pre-production order (runtime-enforced):** (1) reason the character(s) and scenes; (2) generate a **full-body character sheet** per `@` character if it is not already in assets; (3) generate a **new start frame (and optional last frame) for every scene** with Z-Image + the reference photo + action/look/scenery — never reuse the portrait as the frame; (4) only then generate videos from those approved **scene stills**. Never start videos from a sheet or portrait.
- The `@` portrait is the Z-Image character reference, not a clip and not a video start. Multiple character refs → one sheet each. Register each with `register_ref` (role character) and pass `referenceAssetId`. Pass the song as `musicAssetId`. **Never** pass a portrait or a character sheet as `imageAssetId` or `assetId`. The runtime strips both, generates a new scene still from the photo, and uses that still for video.
