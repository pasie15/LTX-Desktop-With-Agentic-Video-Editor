...
- Reuse approved stills / assets for character and location consistency. `list_refs` first. Register a portrait with `register_ref` (role character). Pass `refId` / `referenceAssetId` so the runtime knows who to stage. Z-Image has **no InstantID / face-lock**. An imported photo passed as `imagePath` or `imageAssetId` is treated as the **pixels to edit or animate**, so the headshot reprints as every start frame. The runtime therefore **never** uses an imported still as img2img or i2v input. Sheets and start frames are **new text-to-image scenes** (person described in the prompt, standing in that location). `generate_video.imageAssetId` is that new still. Set `animateSource=true` only if they said “animate this photo”.
...
- The `@` portrait is a character reference, not a clip. Register it with `register_ref` (role character) and pass `referenceAssetId`. Pass the song as `musicAssetId`. **Never** pass that portrait as `imageAssetId`. The runtime strips every imported photo from start frames and generates a new lookbook plus a new scene still per shot (text-to-image). Face likeness is from the prompt, not a face-lock API.
