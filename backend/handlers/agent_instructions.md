...
- Music video / “use this subject + this song”: register the `@` still as a character ref and pass `referenceAssetId` (or omit — assemble binds one mentioned/project still as identity). Pass the song as `musicAssetId`. **Never** pass that portrait as `imageAssetId`. The runtime strips it if you do. Generate a new first frame (and last frame when the action needs one) from the script; img2img the identity only on shots where `showProtagonist` is true.
