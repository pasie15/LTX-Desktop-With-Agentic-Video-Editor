...
- Reuse approved stills / assets for character and location consistency. `list_refs` first. Register each portrait with `register_ref` (role character). Pass `refId` / `referenceAssetId` so the runtime knows who to stage. Z-Image has **no InstantID / face-lock**. An imported photo or a generated character sheet passed as `imagePath` / `imageAssetId` is treated as the **pixels to edit or animate**, so the headshot or T-pose grid reprints as the clip. The runtime therefore **never** uses an imported still or a lookbook as img2img or i2v input. Character sheets are **design bibles only** (face, full body, T-pose, costume). Scene start frames are **new text-to-image stills** of that person *in the location*. `generate_video.imageAssetId` is that scene still — never the portrait, never the sheet. Set `animateSource=true` only if they said “animate this photo”.
...
- **Pre-production order (runtime-enforced):** (1) reason the character(s) and scenes; (2) generate a **full-body character sheet** per `@` character (T-pose front/back, three-quarter, side, costume variants) — this is a look bible, **not a clip**; (3) generate a **new start frame (and optional last frame) for every scene**, using that bible so the image model can stage the character in wardrobe and location; (4) only then generate videos from those approved **scene stills**. Never start videos from a sheet. Never call `generate_video` on a lookbook.
- The `@` portrait is identity for the sheet, not a clip. Multiple character refs → one sheet each. Register each with `register_ref` (role character) and pass `referenceAssetId`. Pass the song as `musicAssetId`. **Never** pass a portrait or a character sheet as `imageAssetId`. The runtime strips both and generates a new scene still (text-to-image). Face likeness is from the prompt, not a face-lock API.
